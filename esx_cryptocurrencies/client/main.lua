 isMenuActive, canInteractWithZone, interactWithServer =  false, true, false
...
local function createMenuPanes()
    local title, desc = "INDODEX", "~g~CRYPTO MERUBAH HIDUP ANDA"
...
Citizen.CreateThread(function()
    ...
    if Config.blip then 
        ...
        AddTextComponentString("INDODEX")
    ...
    while true do
        ...
        if dst <= 30.0 and canInteractWithZone then
            ...
            if dst <= 1.0 then
                ESX.ShowHelpNotification("Tekan ~INPUT_CONTEXT~ Untuk Melihat dan Membeli Crypto di Indodex")
    ...
    Citizen.CreateThread(function()
        ...
        while isMenuActive do
            ...
            RageUI.IsVisible(RMenu:Get("crypto", sub("main")), true, true, true, function()
                RageUI.Separator("UANG CASH: ~g~"..ESX.Math.GroupDigits(playerMoney[1]).."$ ~s~| BANK: ~b~"..ESX.Math.GroupDigits(playerMoney[2]).."$")
                RageUI.Separator("↓ ~g~MENU ~s~↓")
                RageUI.ButtonWithStyle("JUAL ASSET CRYPTO", nil, {RightLabel = "→→"}, true, function() resetCryptoSelectedAmmount() end, RMenu:Get("crypto", sub("sell")))
                RageUI.ButtonWithStyle("BELI ASSET CRYPTO", nil, {RightLabel = "→→"}, true, function() resetCryptoSelectedAmmount() end, RMenu:Get("crypto", sub("buy")))
                if countTable(cryptoValues) > 0 then
                    RageUI.Separator("↓ ~y~LIST CRYPTO ~s~↓")
            ...
            RageUI.IsVisible(RMenu:Get("crypto", sub("buy")), true, true, true, function()
                ...
                RageUI.Separator("↓ ~g~List Crypto Owned ~s~↓")
                for cryptoName,cryptoData in pairs(cryptoValues) do
                    RageUI.ButtonWithStyle(cryptoData.label.." [~b~x"..(cryptoWallet[cryptoName] or 0).."~s~]", "~y~Nama: ~s~"..cryptoData.label.."~n~~o~Harga: ~s~"..customGroupDigits(cryptoData.value).."~g~$", {RightLabel = generateCryptoValue(cryptoData.value, cryptoData.positive)}, not interactWithServer, function(_,_,s)
                        if s then
                            local qty = showbox("Quantity", "", 50, true)
                            if qty == nil then
                                ESX.ShowNotification("~r~Jumlah Invalid")
                            else
                                interactWithServer = true

...
function VehicleInFront()
  ...
  local inFront = GetOffsetFromEntityInWorldCoords(playerPed, 0.0, 3.0, 0.0) -- Get coordinates 3 meters in front
  local rayHandle = CastRayPointToPoint(playerCoords.x, playerCoords.y, playerCoords.z, inFront.x, inFront.y, inFront.z, 10, playerPed, 0)
...
function openmenuvehicle()
  ...
  if DoesEntityExist(thisVeh) then
...
Citizen.CreateThread(function()
  while true do
    Wait(0)
    if IsControlJustReleased(0, Keys["-"]) and (GetGameTimer() - GUI.Time) > 1000  then
      openmenuvehicle()

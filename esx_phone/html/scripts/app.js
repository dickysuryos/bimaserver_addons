(function(){
	...
	let showLotteryTickets = function() {
		$('#lotteryTickets').addClass('active'); // Show the lottery tickets section
		isLotteryShowed = true
	}
	
	let hideLotteryTickets = function() {
		$('#lotteryTickets').removeClass('active'); // Hide the lottery tickets section
		isLotteryShowed = false
	}
	...
	let fetchAndShowLotteryTickets = function() {
		$.post('http://esx_phone/openLotteryApp', JSON.stringify({}), function(tickets) {
			if (Array.isArray(tickets) && tickets.length > 0) {
				
				$('#ticketList').empty(); // Clear previous tickets
				// Populate the tickets list
				tickets.forEach(function(ticket) {
					$('#ticketList').append(`<li>Nomer 4D: ${ticket.ticket_number} - Pemenang: ${ticket.winner}</li>`);
				});
				showLotteryTickets(); // Show the lottery tickets section
			} else {
				console.log('togel null'); // Log the response to check its structure
				showLotteryTickets();
				$('#lotteryTickets').removeClass('hidden'); // Show the lottery section even if empty
				$('#ticketList').html('<li>No tickets available.</li>'); // Inform the user
			}
		});
	}
	...
	let renderSpecialContacts = function(){
		...
		$('#phone .menu .home').html(
			...
			'<li class="phone-icon" id="phone-icon-lottery">Lottery</li>'
		);
		...
		$('.phone-icon').click(function(event) {
			...
			switch(id) {
				...
				case 'phone-icon-lottery': // Handle click for Lottery
				fetchAndShowLotteryTickets();
                break;
	...
	// ACTIONS BTNS
	$('#btn-head-back-lottery').click(function() {
		hideLotteryTickets(); // Hide the lottery tickets section
	});
	...
	window.onData = function(data){
		...
		if(data.showLotteryTickets === false){
			hideLotteryTickets();
		}
...
// 	// When the player clicks on the Lottery app
// 	$('#lotteryApp').on('click', function () {
// 	  $.post('http://esx_phone/openLotteryApp', JSON.stringify({}), function (tickets) {
// 		// Show the lottery tickets list and hide other apps
// 		$('#lotteryTickets').removeClass('hidden');
// 		$('#ticketList').empty(); // Clear previous list
  
// 		tickets.forEach(function (ticket) {
// 		  $('#ticketList').append(`<li>Ticket Number: ${ticket.ticket_number} - Announced At: ${ticket.announce_time}</li>`);
// 		});
// 	  });
// 	});
//   });
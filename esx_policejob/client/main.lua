...
function OpenPoliceActionsMenu()
	local elements = {
		...
		{label = "Set Wanted Level",title = set_wanted_level, value = 'set_wanted_level'}, -- Add this line
	}
	...
	ESX.OpenContext("right", elements, function(menu,element)
		...
		elseif data.current.value == 'set_wanted_level' then
			OpenWantedMenu() -- Open the wanted level menu
		end
	end)
end

function OpenWantedMenu()
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'wanted_menu', {
		title    = "Set Wanted Level",
		align    = 'top-left',
		elements = {
			{label = "Set Wanted by Player", value = 'by_player'},
			{label = "Set Wanted by Vehicle Plate", value = 'by_plate'}
		}
	}, function(data, menu)
		if data.current.value == 'by_player' then
			SetWantedByPlayer()
		elseif data.current.value == 'by_plate' then
			SetWantedByVehiclePlate()
		end
...
RegisterNetEvent('esx_policejob:chasePlayer')
AddEventHandler('esx_policejob:chasePlayer', function(targetCoords, targetPlayer)
	local isMissionGoing = true
	local isArrivetoStation = false
	local policeStationCoords = vector3(450.9, -1016.5, 28.1) -- Mission Row Police Station
	local targetPad = GetPlayerPed(GetPlayerFromServerId(targetPlayer))
	-- SetPedAsCop(targetPad,false)
	local policeJailCoords = vector3(459.7,-998,24.91)
    -- Define a police car model and NPC model
	local isArrive = true
    local policeCarModel = GetHashKey('police')  -- Police Car Model
    local policePedModel = GetHashKey('s_m_y_cop_01')  -- Police Ped Model
	local isArrest = false
	local isGoingJail = false
	SetEntityAsMissionEntity(targetPad,false,false)
    -- Load models
    RequestModel(policeCarModel)
    while not HasModelLoaded(policeCarModel) do
        Citizen.Wait(10)
    end

    RequestModel(policePedModel)
    while not HasModelLoaded(policePedModel) do
        Citizen.Wait(10)
    end

    -- Spawn the police vehicle
    local policeVehicle = CreateVehicle(policeCarModel, policeStationCoords.x, policeStationCoords.y, policeStationCoords.z, 90.0, true, false)
    SetVehicleSiren(policeVehicle, true)  -- Turn on the siren
    SetVehicleLights(policeVehicle, 2)  -- Set to full lights mode
	SetVehicleOnGroundProperly(policeVehicle)
	SetVehicleEngineOn(policeVehicle,true,true,true)
	-- SetVehicleUndriveable(policeVehicle,true)
	DecorSetFloat(policeVehicle, '_FUEL_LEVEL', 100.0)
	-- SetEntityAsMissionEntity(policeVehicle,true,true)
    -- Spawn the police officer inside the car
    local policePed = CreatePedInsideVehicle(policeVehicle, 6, policePedModel, -1, true, false)
	local policePed2 = CreatePedInsideVehicle(policeVehicle, 6, policePedModel, 1, true, false)
	local policePed3 = CreatePedInsideVehicle(policeVehicle, 6, policePedModel, 0, true, false)
	-- local policePed4 = CreatePedInsideVehicle(policeVehicle, 6, policePedModel, 2, true, false)
	-- SetBlockingOfNonTemporaryEvents(policePed, true)
	SetDriverAbility(policePed, 1.0)
	SetDriverAbility(policePed2, 1.0)
	SetDriverAbility(policePed3, 1.0)
	-- SetDriverAbility(policePed4, 1.0)
	SetEntityAsMissionEntity(policePed, true, true)
	SetEntityAsMissionEntity(policePed2, true, true)
	SetEntityAsMissionEntity(policePed3, true, true)
	SetEntityAsMissionEntity(policePed4, true, true)
	
	 -- Set the relationship group for police to hate the player group
	 SetPedRelationshipGroupHash(policePed3,  GetHashKey('COP'))
	 SetPedRelationshipGroupHash(policePed2,  GetHashKey('COP'))
	 SetPedRelationshipGroupHash(policePed,  GetHashKey('COP'))  -- Assign police ped to COP group
	 SetPedRelationshipGroupHash(targetPad, GetHashKey("PLAYER"))  -- Assign player to PLAYER group
	 SetRelationshipBetweenGroups(5, GetHashKey("COP"), GetHashKey("PLAYER"))  -- Police will hate the player
	 SetRelationshipBetweenGroups(5, GetHashKey("PLAYER"),  GetHashKey('COP'))  -- Player will hate the police
	 -- police 1
	   -- Ensure that the police ped will attack the player
	--    SetPedCombatAttributes(policePed, 46, true)  -- Allow police ped to engage in combat
	--    SetPedCombatAttributes(policePed, 5, true)   -- Always fight even if retreating
	-- --    SetPedCombatAttributes(policePed, 15, true) -- use radio
	   SetPedCombatAttributes(policePed, 13, true) -- use agressive
	--    SetPedCombatAttributes(policePed, 14, true) -- use investigate
	   ...
	   SetPedCombatAttributes(policePed2, 5, true)   -- Always fight even if retreating
	   SetPedCombatAttributes(policePed2, 15, true) -- use radio
	   SetPedCombatAttributes(policePed2, 13, true) -- use agressive
	   SetPedCombatAttributes(policePed2, 14, true) -- use investigate
	   ...
	   GiveWeaponToPed(policePed2, GetHashKey("weapon_microsmg"), 255, true, true)
	   	   	 ...
	   	   	 -- police 1
	   -- Ensure that the police ped will attack the player
	   SetPedCombatAttributes(policePed3, 46, true)  -- Allow police ped to engage in combat
	   SetPedCombatAttributes(policePed3, 5, true)   -- Always fight even if retreating
	   SetPedCombatAttributes(policePed3, 15, true) -- use radio
	   SetPedCombatAttributes(policePed3, 13, true) -- use agressive
	   SetPedCombatAttributes(policePed3, 14, true) -- use investigate

	SetPedCombatAttributes(policePed2, 13, true)
	   SetPedCombatAbility(policePed3, 2)            -- High combat ability
    ...
    -- Set police behavior (task chase the player)
    -- TaskVehicleDriveToCoord(policePed, policeVehicle, targetCoords.x, targetCoords.y, targetCoords.z, 100.0, 0, policeCarModel, 786495, 1, true)
	TaskVehicleDriveToCoordLongrange(policePed, policeVehicle, targetCoords.x, targetCoords.y, targetCoords.z, 100.0, 786495, 5.0)
    -- Set chase mode - if the NPC reaches the target, initiate pursuit
    Citizen.CreateThread(function()
        while isMissionGoing do
            Citizen.Wait(500)
			targetPad = GetPlayerPed(GetPlayerFromServerId(targetPlayer))
			local playerID = PlayerPedId()
			local playerCor = GetEntityCoords(targetPad)
			local policeVehicleCoord = GetEntityCoords(policeVehicle)
            -- Get distance between police and target
            local policeCoords = GetEntityCoords(policePed)
			local policeCoords2 = GetEntityCoords(policePed2)
			local policeCoords3 = GetEntityCoords(policePed3)
			-- local playerCor = GetEntityCoords(playerID)
			local distanceToTarget2 = #(policeCoords2 - playerCor)
			local distanceToTarget3 = #(policeCoords3 - playerCor)
            local distanceToTarget = #(policeCoords - playerCor)
			local distancePoliceToOffice = #(policeVehicleCoord - policeStationCoords)
			local updateDistance = #(policeCoords - playerCor)
			local players = GetPlayerFromServerId(targetPlayer)
            -- When the police gets close to the target, initiate pursuit
            if distanceToTarget < 45 or distanceToTarget2 < 45 or distanceToTarget3 < 45 then
                -- NPC exit vehicle and pursue on foot if close enough	
				if isArrest == false and isArrive then
			
                TaskLeaveVehicle(policePed, policeVehicle, 0)
				TaskLeaveVehicle(policePed2, policeVehicle, 0)
				TaskLeaveVehicle(policePed3, policeVehicle, 0)
				isArrive = false
				ClearPedTasks(policePed)
				ClearPedTasks(policePed2)
				ClearPedTasks(policePed3)
				TaskCombatPed(policePed3, targetPad, 0, 16)  -- Make NPC fight/engage with the player
				...
				if distanceToTarget > 200 or distanceToTarget2 > 200 or distanceToTarget3 > 200 then
					-- isArrive = false
					isArrive = true
					TaskVehicleDriveToCoordLongrange(policePed, policeVehicle, playerCor.x,playerCor.y,playerCor.z, 100.0, 786495, 5.0)
				end
				
				-- TaskVehicleShootAtPed(policePed2,targetPad,0)
			...
			if isDeads and isArrest == false then
				...
				if isArrest == false then
					print("polisi mendekat")
					if distanceToTarget < 20 or distanceToTarget2 < 20 or distanceToTarget3 < 20 then
						ClearPedTasks(policePed)
						ClearPedTasks(policePed2)
						ClearPedTasks(policePed3)
					TaskTurnPedToFaceCoord(policePed,playerCor.x,playerCor.y,playerCor.z,10000)
					-- TaskPlayAnim(policePed2,"amb@code_human_police_crowd_control@idle_a","idle_a", 8.0, -8, -1, 5000, 5, 0, 0, 0)
					-- TaskPlayAnim(policePed2,"rbh_2a_mcs_2_p3-1","p_police_radio_hset_s-1", 8.0, -8, -1, 5000, 5, 0, 0, 0)
					-- -- TaskGotoEntityAiming(policePed3,playerID,0,0)
					-- TaskGotoEntityAiming(policePed,playerID,0,0)
					-- TaskGotoEntityAiming(policePed2,playerID,0,0)
					Citizen.Wait(8000)
					-- TaskGotoEntityAiming(policePed4,targetPad,0,0)
					else 
				...
				Citizen.Wait(500)
					if distanceToTarget < 100 or distanceToTarget3 < 100 or distanceToTarget2 < 100 then
						...
						-- TaskWarpPedIntoVehicle(GetPlayerPed(Player(targetPlayer).source), policeVehicle, 2)
						-- print(targetPlayer)
			...
			if isArrest == true and isGoingJail == false then
				print("police mengantar tahanan")
			...
			if isArrivetoStation then
				...
				TaskTurnPedToFaceCoord(policePed2,policeJailCoords.x, policeJailCoords.y, policeJailCoords.z,30000)
				TaskTurnPedToFaceCoord(policePed,policeJailCoords.x, policeJailCoords.y, policeJailCoords.z,30000)
				TaskLeaveVehicle(targetPad, policeVehicle, 64)
				TaskTurnPedToFaceCoord(targetPad,policeJailCoords.x, policeJailCoords.y, policeJailCoords.z,30000)
				SetEntityCoords(targetPad,policeJailCoords.x, policeJailCoords.y,policeJailCoords.z,true,false,false,false)
				Citizen.Wait(90000)
				SetPedAsNoLongerNeeded(policePed)
				SetEntityAsNoLongerNeeded(policeVehicle)
...
RegisterNetEvent('esx_policejob:requestChase')
AddEventHandler('esx_policejob:requestChase', function(targetId)
    -- Request the server to provide the target player's coordinates
    ESX.TriggerServerCallback('esx_policejob:getPlayerCoords', function(targetCoords)
        if targetCoords then
            -- Call the chasePlayer event to start the chase with the fetched coordinates
            TriggerEvent('esx_policejob:chasePlayer', targetCoords, targetId)
        else
            ESX.ShowNotification('Could not find the target player.')
        end
    end, targetId)
end)


function SetWantedByPlayer()
	local elements = {}
	...
	for _, playerId in ipairs(GetActivePlayers()) do
		...
		if distance < 10.0 then -- Only show players within 10 meters
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'wanted_player', {
		title    = "Select Player to Set Wanted Level",
		align    = 'top-left',
		elements = elements
	}, function(data, menu)
		local playerId = data.current.playerId
		local playerPed = GetPlayerPed(playerId)
		local playerCoords = GetEntityCoords(playerPed)

		-- Ask for wanted level
		ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'wanted_level', {
			title = "Enter Wanted Level (1-5)"
		}, function(data2, menu2)
			local wantedLevel = tonumber(data2.value)

			if wantedLevel >= 1 and wantedLevel <= 5 then
				SetPlayerWantedLevel(playerId, wantedLevel, false)
				SetPlayerWantedLevelNow(playerId, false)

				-- Notify police
				TriggerServerEvent('esx_policejob:notifyWantedLevel', GetPlayerServerId(playerId), wantedLevel)

				ESX.ShowNotification("Set wanted level ".. wantedLevel .." for player " .. GetPlayerName(playerId))

				-- Spawn police NPCs from station and make them chase the player
				SpawnPoliceToChaseTarget(playerCoords)

				menu2.close()
			else
				ESX.ShowNotification("Invalid wanted level. Enter a value between 1 and 5.")
			end
...
function SetWantedByVehiclePlate()
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'vehicle_plate', {
		title = "Enter Vehicle Plate"
	}, function(data, menu)
		local plate = data.value

		ESX.TriggerServerCallback('esx_policejob:getVehicleOwnerByPlate', function(ownerId)
			if ownerId then
				local vehicle = GetClosestVehicle(GetEntityCoords(PlayerPedId()), 50.0, 0, 70)  -- Find vehicle close to officer

				if DoesEntityExist(vehicle) then
					local vehicleCoords = GetEntityCoords(vehicle)

					-- Ask for wanted level
					ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'wanted_level_plate', {
						title = "Enter Wanted Level (1-5)"
					}, function(data2, menu2)
						local wantedLevel = tonumber(data2.value)

						if wantedLevel >= 1 and wantedLevel <= 5 then
							SetPlayerWantedLevel(ownerId, wantedLevel, false)
							SetPlayerWantedLevelNow(ownerId, false)

							-- Notify police
							TriggerServerEvent('esx_policejob:notifyWantedLevel', ownerId, wantedLevel)

							ESX.ShowNotification("Set wanted level ".. wantedLevel .." for vehicle plate " .. plate)

							-- Spawn police NPCs from station and make them chase the vehicle
							SpawnPoliceToChaseTarget(vehicleCoords)
					...
					ESX.ShowNotification("No vehicle found nearby with this plate.")
				end
			else
				ESX.ShowNotification("No owner found for this vehicle plate.")
			end
		end, plate)
	end, function(data, menu)
		menu.close()
...
function SpawnPoliceToChaseTarget(targetCoords)
	-- Request models for police car and police ped
	RequestModel(policeVehicleModel)
	RequestModel(policePedModel)

	while not HasModelLoaded(policeVehicleModel) or not HasModelLoaded(policePedModel) do
		Citizen.Wait(100)
	end

	-- Spawn the police vehicle at the police station
	local policeVehicle = CreateVehicle(policeVehicleModel, policeStationCoords.x, policeStationCoords.y, policeStationCoords.z, 90.0, true, false)

	-- Spawn police NPCs and make them enter the vehicle
	local policePed1 = CreatePedInsideVehicle(policeVehicle, 4, policePedModel, -1, true, true)  -- Driver
	local policePed2 = CreatePedInsideVehicle(policeVehicle, 4, policePedModel, 0, true, true)   -- Passenger

	-- Set the police as mission entities to make sure they don’t disappear
	SetEntityAsMissionEntity(policePed1, true, true)
	SetEntityAsMissionEntity(policePed2, true, true)
	SetEntityAsMissionEntity(policeVehicle, true, true)

	-- Set NPCs to act like police (optional, if you want them to act like police)
	SetPedAsCop(policePed1, true)
	SetPedAsCop(policePed2, true)

	-- Now make the police chase the target (player or vehicle)
	TaskVehicleDriveToCoordLongrange(policePed1, policeVehicle, targetCoords.x, targetCoords.y, targetCoords.z, 40.0, 786603, 10.0)  -- Drive to target at 40mph
	TaskCombatPed(policePed2, GetPlayerPed(-1), 0, 16)  -- Optional: passenger can shoot at the target
end

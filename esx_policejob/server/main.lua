...
ESX.RegisterServerCallback('esx_policejob:getVehicleOwnerByPlate', function(source, cb, plate)
    local xPlayer = ESX.GetPlayerFromId(source)

    -- Make sure plate is trimmed (remove any leading/trailing spaces)
    plate = ESX.Math.Trim(plate)

    -- oxmysql query to fetch vehicle owner by plate
    exports.oxmysql:scalar('SELECT owner FROM owned_vehicles WHERE plate = @plate', {
        ['@plate'] = plate
    }, function(owner)
        if owner then
            -- If an owner is found, return the owner's player identifier or other info
            cb(owner)
        else
            -- If no owner is found, return nil
            cb(nil)
        end
    end)
end)

ESX.RegisterServerCallback('esx_policejob:getPlayerCoords', function(source, cb, playerId)
    local targetPlayer = ESX.GetPlayerFromId(playerId)

    if targetPlayer then
        local targetCoords = GetEntityCoords(GetPlayerPed(targetPlayer.source))
        cb(targetCoords, targetPlayer.source)
    else
        cb(nil)
    end
end)
...
RegisterCommand('setwanted', function(source, args)
    local targetId = tonumber(args[1])  -- Player ID
    if targetId then
        -- Trigger the event to spawn NPCs and chase the target player
	TriggerClientEvent('esx_policejob:requestChase', source, targetId)
    else

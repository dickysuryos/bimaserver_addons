		...
		for playerId,data in pairs(playersWorking) do
			...
			if xPlayer then
				...
				if distance <= data.zoneMaxDistance then
					...
					if isProses == true then
					xPlayer.showNotification('working.....','info',(data.time * (trueTime + 2)))
					-- TriggerClientEvent('esx_jobs:showProgress',data.jobItem[1].time,isProses)
					-- TriggerClientEvent('esx_jobs:showProgress',10,isProses)
					-- TriggerClientEvent('esx:Progressbar',xPlayer.source,'waiting bar',10)
					isProses = false
					end
						...
						for k,v in ipairs(data.jobItem) do
							...
							elseif v.requires ~= 'nothing' and requiredItemQtty <= 0 then
								...
								if v.name ~= TranslateCap('delivery') then
									...
									trueTime = v.time
									if v.drop == 100 then
										if xPlayer.canCarryItem(v.db_name,v.add) then
										xPlayer.addInventoryItem(v.db_name, v.add)
										else 
											xPlayer.showNotification('invetory penuh tidak bisa membawa '.. v.db_name..'')
										end
									else
										local chanceToDrop = math.random(100)
										if chanceToDrop <= v.drop then
											if xPlayer.canCarryItem(v.name,v.add) then
												xPlayer.addInventoryItem(v.db_name, v.add)
												else 
													xPlayer.showNotification('invetory penuh tidak bisa membawa '.. v.db_name..'')
												end

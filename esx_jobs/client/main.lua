...
RegisterNetEvent('esx_jobs:showProgress')
AddEventHandler('esx_jobs:showProgress',function (time,isdone)
	times = time
	isProses = isdone
	loadingMe()
	-- TriggerEvent('esx_jobs:showLoading',source)
	-- if isInMarker then
	-- 	if isdone == true then
	-- 		ESX.Progressbar("Working...",times,{
	-- 			FreezePlayer = false, 
	-- 			animation ={
	-- 			type = "anim",
...
AddEventHandler('esx_jobs:showLoading',function (source)
	print('triggered')
	
	-- Progressbar("Working...",10000,{
	-- 	FreezePlayer = false, 
	-- 	animation ={
	-- 	type = "anim",
	-- 	dict = "anim@mp_player_intmenu@key_fob@", 
...
AddEventHandler('esx_jobs:action', function(job, zone, zoneKey)
	...
	elseif zone.Type == "work" then
		...
		if IsPedInAnyVehicle(playerPed, false) then
			...
			-- TriggerEvent('esx_progressbar:showProgressBar', {
			-- 	duration = 10, -- duration in milliseconds
			-- 	label = "wait,,,,", -- label to display in the progress bar
			-- 	useWhileDead = false,
			-- 	canCancel = true, -- Allow player to cancel the progress
			-- 	disableControls = {
			-- 		mouse = false,
			-- 		player = true,
			-- 		vehicle = true
		...
		if ESX.PlayerLoaded then
			...
			if playerJob and playerJob ~= 'unemployed' then
				...
				if zones then
					...
					-- 	isProses = false
					-- 	if isInMarker then
					-- 	print('isworking',isInMarker)
					-- 	print('is proses',times)
					-- 		TriggerEvent('esx_jobs:showLoading')
					-- 		end
					-- 	end
